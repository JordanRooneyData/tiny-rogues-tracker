...
def test_gui_source_uses_pyside6_and_required_views():
    ...
    assert "Character Records" in source
    assert "Completion Counts" in source
    assert "Character Run Matrix" in source
...
def test_build_release_update_contract_files():
    ...
    assert "PyInstaller" in ps1 and "TinyRoguesTracker-v0.4.0" in ps1
    iss = (ROOT / "installer" / "TinyRoguesTracker.iss").read_text(encoding="utf-8")
    assert "AppVersion=0.4.0" in iss
