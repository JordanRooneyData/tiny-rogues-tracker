...
def release(tag="v0.5.0", draft=False, prerelease=False, assets=None, body="## Fixes\nWorking auto-update"):
    return {
        "tag_name": tag,
        "draft": draft,
        ...
        "assets": assets if assets is not None else [
            {"name": "TinyRoguesTracker-v0.5.0.exe", "browser_download_url": "https://example/app.exe"},
            {"name": "TinyRoguesTracker-v0.5.0-Setup.exe", "browser_download_url": "https://example/setup.exe"},
        ],
    }
...
def test_043_detects_v0431_as_newer_and_leading_v_is_robust():
    info = updater.update_info_from_release_payload(release("v0.5.0"), current_version="0.4.3")
    assert info is not None
    assert info.latest_version == "0.5.0"
    assert info.asset_name == "TinyRoguesTracker-v0.5.0-Setup.exe"
    assert updater.is_newer_version("0.4.3", "v0.5.0")


def test_equal_and_older_remote_versions_do_not_trigger_update():
    assert updater.update_info_from_release_payload(release("v0.5.0"), current_version="0.5.0") is None
    assert updater.update_info_from_release_payload(release("v0.4.1"), current_version="0.5.0") is None
...
def test_correct_installer_asset_selection_prefers_setup_not_standalone_exe():
    ...
    chosen = updater.select_installer_asset(assets, "0.5.0")
    assert chosen["name"] == "TinyRoguesTracker-v0.5.0-Setup.exe"


def test_missing_installer_asset_is_recoverable_error(tmp_path):
    info = updater.update_info_from_release_payload(release(assets=[{"name": "TinyRoguesTracker-v0.5.0.exe", "browser_download_url": "https://example/app.exe"}]), current_version="0.4.1")
...
def test_failed_download_does_not_require_app_crash(tmp_path):
    info = updater.UpdateInfo("0.4.1", "0.5.0", "", asset_name="TinyRoguesTracker-v0.5.0-Setup.exe", asset_url="https://example/fail.exe")
...
def test_download_validation_requires_non_empty_windows_executable(tmp_path):
    good = tmp_path / "TinyRoguesTracker-v0.5.0-Setup.exe"
...
def test_installer_appid_install_path_stable_and_relaunches_after_upgrade():
    ...
    assert "TinyRoguesTracker-v0.5.0-Setup" in iss
